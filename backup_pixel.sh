...
DATA_BACKUP_DIR="/home/tyler/backup/pixel8/${CURR_DATETIME}"
...
MOTO_EXTRACAM_DIR="${MOTO_DCIM_DIR}/OpenCamera"
MOTO_CAM_DIR="${MOTO_DCIM_DIR}/Camera"
# MOTO_PRIVATE_DIR="${MOTO_MAIN_STORAGE_DIR}/.toozexy"
MOTO_MOVED_CAM_DIR=$(join_by / "${MOTO_MAIN_STORAGE_DIR}" "DCIM" "${CURR_DATE}")
...
# MOTO_DCIM_SUBDIRS=( "PhotosEditor" "Restored" "Screen recordings" "Screenshots" "Tiktok" )
...
# OpenCamera
pull_folder "${MOTO_EXTRACAM_DIR}" "${PIC_BACKUP_DIR}"
echo "Moving OpenCamera dir ${MOTO_EXTRACAM_DIR} to ${MOTO_MOVED_CAM_DIR}"
adb shell mv "${MOTO_EXTRACAM_DIR}" "${MOTO_MOVED_CAM_DIR}"
...
#     fi
# done
# TODO: remove files from motofor picdir in $(adb shell ls $(escape_spaces "${MOTO_PIC_DIR}")); do
#     picpath=$(join_by / "${MOTO_PIC_DIR}" "${picdir}")
#     pull_and_delete "${picpath}" "${PIC_BACKUP_DIR}"
#     # exit_if_fail "adb pull ${picpath}"
# done
...
MOTO_TOP_LEVEL_DIRS=( "AMdroid" "beam" "Documents" "Download" "games" "Playlists" "SmsContactsBackup" "VisualVoiceMail" "Voicemails" "WhatsApp" )
